...
class CounterSuite extends FunSuite {
  ...
  test("Counter inc and dec work without parameters") {
    assert(new Counter(10).inc().dec().inc().inc().count == 12)
  }
  test("Counter inc and dec work with parameters") {
    assert(new Counter(10).inc(4).dec(3).inc(2).inc(1).count == 14)
  }

  test("CaseCounter inc and dec work without parameters") {
    assert(new CaseCounter(10).inc().dec().inc().inc().count == 12)
  }
  test("CaseCounter inc and dec work with parameters") {
